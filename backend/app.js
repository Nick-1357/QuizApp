...
const url = 'mongodb+srv://quizAppMobileUser:xkLaBBaa1dTsZuX8@quizappdb.7ltdg.mongodb.net/quizAppDummyData';
...
app.post('/post', async function(req, res){
    ...
    try {
        ...
        res.status(500).json({"success": false, "message":"Error in saving question details"});
    }
});
...
app.post('/updatePassword/:userId/:password', async function(req, res){
    try{
        await User.findOneAndUpdate({userId: req.params.userId}, {password: req.params.password});
        res.status(200).send({
            message: "question with id: "+req.params.userId+" updated successfully"
        });
    }
    catch(err) {
        res.status(500).send({
...
app.post('/updateUserName/:userId/:name', async function(req, res){
    try{
        await User.findOneAndUpdate({userId: req.params.userId}, {name: req.params.name});
        res.status(200).send({
            message: "question with id: "+req.params.userId+" updated successfully"
        });
    }
...
app.put('/updateQuestionId/:id/:questionId', async function(req, res){
    ...
    Question.findByIdAndUpdate(req.params.id, {questionId: req.params.questionId})
    .then( () => {
        res.status(200).send({
...
app.post('/recordAnswers', async function(req, res){
    ...
    try {
        ...
        res.status(200).json({"message":"User details saved", "objectSaved": quizattempt});
    } catch(err) {
        res.status(500).json({ "message":"Error in saving user details", "error": err});
    }
});
