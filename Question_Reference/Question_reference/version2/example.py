...
def runExampleC(q, chapter):
    ...
    for w in additionalkw:
        ...
        weight.append(w[1])
    print(kws)
    res=calculatePharagraph(keywords(words, weight),SlideWindow.text.splitByComma(text))

    max = 0
    index = 0
    for i in range(len(res)):
        if res[i].weight>res[index].weight:
            index = i
    ...
    for i in range(len(res)):
        if(i<index+3 and i>index-3):
            print("\nIndex:{}".format(i))
            print(res[i].content)
            returnText = returnText+'<br><br>'+res[i].content
        i=i+1
    return returnText
